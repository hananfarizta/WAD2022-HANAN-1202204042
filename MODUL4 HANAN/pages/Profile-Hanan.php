...
if (isset($_SESSION["login"])) {
    ...
    header("Location: Login-Hanan.php");
    exit;
}
...
if (isset($_POST["update"])) {
    ...
    $konfirmasi_password = mysqli_real_escape_string($koneksiuser, $_POST["password2"]);
    setcookie("warna_navbar", $_POST["warna_navbar"], time() + 86400, "/");

    if ($password == $konfirmasi_password) {
        $query = "UPDATE user_hanan SET
                nama = '$nama',
                no_hp = '$no_hp',
                password = '$password'
              WHERE email = '$email'";
        mysqli_query($koneksiuser, $query);
    }
...
<body>
    ...
    <nav class="navbar navbar-expand navbar-dark bg-<?= isset($_COOKIE["warna_navbar"])  ? $_COOKIE["warna_navbar"] : "primary"; ?>">
        <div class="container py-2">
            <div class="navbar-nav">
                ...
                <a class="nav-link" href="List-Hanan.php">MyCar</a>
            </div>
            <div class="d-flex">
                <a href="Add-Hanan.php" class="btn btn-light text-<?= isset($_COOKIE["warna_navbar"])  ? $_COOKIE["warna_navbar"] : "primary"; ?>" role="button">Add Car</a>
    ...
    <div class="container mt-4">
        ...
        <div class="d-flex align-items-center logo-ead">
            ...
            <span class="ms-5 text-muted">Hanan_1202204042</span>
        </div>
    </div>
